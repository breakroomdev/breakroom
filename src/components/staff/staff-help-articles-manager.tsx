...
export function StaffHelpArticlesManager({ initialArticles }: { initialArticles: HelpArticleRow[] }) {
  const [articles, setArticles] = React.useState(initialArticles);
  const [deleteTarget, setDeleteTarget] = React.useState<HelpArticleRow | null>(null);
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/staff/help-articles/${deleteTarget.id}`, { method: "DELETE" });
      if (!res.ok) {
  ...
  return (
    <div>
      <div className="mb-4 flex justify-end">
        <Button asChild>
          <Link href="/staff/help-articles/new">
      ...
      {articles.length === 0 ? (
        <EmptyState
          ...
          description="Write the first doc for the public help center."
          action={
            <Button asChild>
              <Link href="/staff/help-articles/new">
                <Plus className="h-4 w-4" /> New article
            ...
            <Card key={a.id}>
              <CardContent className="flex items-center gap-3 p-3">
                ...
                <div className="min-w-0 flex-1">
                  <Link href={`/staff/help-articles/${a.id}`} className="truncate font-medium hover:underline">
                    {a.title}
                  </Link>
