...
export interface HelpArticleInitial {
  id: string;
  title: string;
  slug: string;
  content: string;
...
export function StaffHelpArticleEditor({ initial }: { initial: HelpArticleInitial | null }) {
  ...
  async function save() {
    ...
    try {
      ...
      const res = initial
        ? await fetch(`/api/staff/help-articles/${initial.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
        : await fetch(`/api/staff/help-articles`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      ...
      router.push("/staff/help-articles");
      router.refresh();
    } finally {
      setSaving(false);
    }
  ...
  async function remove() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/staff/help-articles/${initial.id}`, { method: "DELETE" });
      if (!res.ok) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">{initial ? "Update this help center article." : "Write a new public help doc."}</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <Field label="Title" htmlFor="help-title">
            <Input id="help-title" value={title} onChange={(e) => updateTitle(e.target.value)} maxLength={120} />
          </Field>
          <Field label="URL" htmlFor="help-slug" hint={`/help/${slug || "…"}`}>
            <Input
              id="help-slug"
              value={slug}
              onChange={(e) => {
          ...
          <Field label="Category" htmlFor="help-category" hint="Optional">
            <Input id="help-category" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Getting started" maxLength={60} />
          </Field>
          <label className="flex items-center justify-between rounded-lg border border-border p-3">
            <div>
              <p className="text-sm font-medium">Published</p>
              <p className="text-xs text-muted-foreground">Drafts are only visible here in the staff panel.</p>
            </div>
